using UnityEngine;
using UnityEngine.UI; // UI Image 및 Text 컴포넌트 제어를 위해 유지
...
public class SkillManager : MonoBehaviour
{
    ...
    [Header("중앙 정보 표시 UI (⭐ 토글 연동 기능 추가)")]
    [SerializeField] private Text centerTopText;    // 하이어라키의 '사용 가능한 슬롯 수' 위치의 Text
    [SerializeField] private Text centerCostText;   // 하이어라키의 '스킬 코스트' 위치의 Text

    // 각 스킬 슬롯의 Skill_Mask Image 컴포넌트를 순서대로 담아둘 배열
    private Image[] maskImages = new Image[5];

    // 각 스킬 슬롯의 Skill_Mask180 GameObject를 순서대로 담아둘 배열
    ...
    private void UpdateSkillMasks()
    {
        ...
        // MP가 최대치(500) 미만일 때만 Skill_MaskGroup을 활성화
        if (skillGroupTransform != null)
        {
