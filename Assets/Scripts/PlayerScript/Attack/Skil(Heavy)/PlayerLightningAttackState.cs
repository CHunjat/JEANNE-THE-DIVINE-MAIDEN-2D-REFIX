...
public class PlayerLightningAttackState : PlayerAttackState
{
    ...
    public override void LogicUpdate()
    {
        ...
        // 애니메이션이 45% 이상 진행되었을 때 평타 키를 누르면 즉시 1타로 연계!
        if (GetNormalizedTime() >= 0.556f && player.inputReader.AttackPressed) //계산해서 0.667이면 대쉬공격이 끝나는 모션시작할때임.
        {
            player.inputReader.AttackPressed = false; // 입력 소비 (중복 발동 방지)
            Debug.Log("스킬 후딜레이 캔슬! 평타 1타로 즉시 연계!");

            stateMachine.ChangeState(player.Attack1State);
            return;
        }

        // 아무 입력 없이 스킬 애니메이션이 100% 끝나면 자연스럽게 Idle로 복귀
