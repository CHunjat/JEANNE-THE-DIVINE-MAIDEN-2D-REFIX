...
public class MidBoss : EnemyFSM
{
    ...
    {
        // 2페이즈로 변신하는 연출 중이라면 데미지를 안 받고 함수를 종료함 (무적 처리).
    ...
    {
        // 기획서상 보스는 경직 면역이므로 상태가 Hit로 바뀌면 안 됨.
        // 따라서 이 안은 비워둠. 시각적 하얀색 피격 효과는 저 위 TakeDamage()에서 이미 처리함.
    }
    ...
    protected override void OnDead()
    {
        ...
        // 2. 시체에 플레이어가 걸려 넘어지거나 비비적거리는 걸 막기 위해 충돌체(Collider)를 꺼버림.
        ...
        // 4. (참고) 만약 죽고 나서 3초 뒤에 시체를 화면에서 아예 지워버리고 싶으면 아래 주석을 풀기.
